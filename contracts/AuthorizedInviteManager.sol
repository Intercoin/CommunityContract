    ...
    CostManagerHelper
{
    ...
    function invitePrepare(bytes memory sSig, bytes memory rSig) external {
        ...
        require(
            inviteSignatures[sSig].exists == false,
    ...
    ) external //nonReentrant()
    {
        ...
        address releaseManagerAddr = ReleaseManagerHelper(deployer)
            .releaseManager();
        bool isCommunityVerifying = IReleaseManager(releaseManagerAddr)
            .checkInstance(communityDestination);
        ...
        if (
            pAddr == address(0) ||
            rpAddr == address(0) ||
            keccak256(abi.encode(signature.rSig)) !=
            keccak256(abi.encode(rSig)) ||
            rpDataArr[0].parseAddr() != rpAddr ||
            !isCommunityVerifying ||
            keccak256(abi.encode(str2num(dataArr[3]))) !=
        ...
        uint8[] memory rolesIndexesTmp = ICommunity(communityDestination)
            .getRolesWhichAccountCanGrant(pAddr, rolesArr);
    ...
    function getInviteReservedHash2(
        bytes memory sSig,
        bytes memory rSig
    ) public pure returns (bytes memory, bytes memory, bytes memory, bytes32) {
        return (
            sSig,
            rSig,
