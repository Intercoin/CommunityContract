...
contract CommunityHookBad is CommunityHookBase {
    ...
    function roleGranted(
        ...
        if (throwInRoleGrantedExecuted) {
            revert("error in granted hook");
        }
    }
    ...
    function roleRevoked(
        ...
        if (throwInRevokedExecuted) {
            revert("error in revoked hook");
        }
    }

    function roleCreated(bytes32 role, uint8 roleIndex) external {
        if (throwInCreatedExecuted) {
            revert("error in created hook");
        }
    }
}
